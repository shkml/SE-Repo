...
def index():
    ...
    center = station_coord[0]  # 'center' variable is used to generate the map as the center of the generated map.
    lat0 = center['lat']
    lng0 = center['lng']
...
def make_markers():
    # define a function to generate the information box which will be shown in the map combined with location icons
    # this function will be used in the function 'singleshow()'
...
def add_marker():
    # define a function to generate the information box which will be shown in the map combined with location icons
    result = bike_data_fetch()  # fetch the realtime data
    infobox_list = []
...
def singleshow():
    # define a function to generate the information box for a specific station according to station name passed from
    # front-end web page.
    sel_value = request.args.get('selected_value')  # get the passed station from front-end
    if sel_value == 'ALL':  # if no specific station is selected, the sel_calue will be 'ALL'
        infobox_list = make_markers()
        return jsonify(infobox_list)
    else:
        result = bike_data_fetch()  # fetch the  real time data about the stations
        station = {}
        for i in result:
            if i["name"] == sel_value:
                station = i
...
# Define a function to fetch the present weather data using api
def getweather():
    W_APIKEY = 'c9d5929c3180f174f633828540c0fbc5'
    sel_station = request.args.get("selected_sta") # get the passed station from front-end
    sta = {}
    result = bike_data_fetch()
    if sel_station == 'ALL':
        sta = result[0]
    ...
    h_temp = weather_result["main"].get("temp")  # get tempetature
    weather_dict["temp"] = str(round(h_temp - 273.15)) + '˚C'
    weather_dict["main"] = str(weather_result["weather"][0].get('main'))    # get weather description
    weather_dict["wind"] = str(weather_result["wind"].get("speed")) + 'm/s' # get wind speed
    print(weather_dict)
...
def graphdata():
    # define a function to get the data that will be shown in the form of graph.
    try:
        bd_json = open('./static/processedBikeData.json')   # all necessary data is saved in file processedBikeData.json
    except IOError:
        print("file dublin.json not found")
    else:
        print('successfully open file dublin.json ')
    ...
    staname = request.args.get("selected_sta")  # get the selected station name from front-end
    GRAPHDATA = {}
    for i in gdata:
        if i == staname:
            print(gdata[i])
...
def showprediction():
    # define a function to get the prediction data using ML model.
    # For available stands, the parameters that the ML model need are 'hour', temperature, weekday.
    # For available bikes, the parameters that the ML model need are 'hour', temperature, available stands, weekday.

    # get station name and selected time from front-end
    staname = request.args.get('staname')   # get station name
    Time = float(request.args.get('time'))  # get prediction time at which the user want to use dublinbike.
    Time /= 1000
    print(Time)

    # get ML parameter 'sta_num'
    sta = {}
    result = bike_data_fetch()
    for i in result:
        if i["name"] == staname:
    ...
    # get temp
    WP_APIKEY = '9570260da25526e20bf66bdf7e1c25e5'  # this api key is used to get weather forecast.
    weather_p = requests.get(
        "http://api.openweathermap.org/data/2.5/forecast?lat={}&lon={}&appid={}".format(LAT, LNG, WP_APIKEY))
    weather_result = json.loads(weather_p.text)  # get weather forecast data
    ...
    # get hour
    hour = (time.localtime(Time)).tm_hour   # get hour
    print(hour)
    # get hour list for the parameter of prediction model
    hour_l = [0, 0, 0]
    if hour > 11:
        hour_l[2] = 1
        hour_l[0] = hour - 12
    ...
    print([hour_l + weeklist])  # [[0, 1, 0, 0, 0, 0, 0, 0, 1, 0]] means 0 o'clock am , Saturday
    ...
    print(paralist)  # [0, 1, 0, 1.81, 20, 0, 0, 0, 0, 0, 1, 0](means 0 o'clock, am, 1.81˚C, 20 avaliable stands, Saturday )
